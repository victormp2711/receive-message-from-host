...
public class MessageVerticle extends AbstractVerticle {
    ...
    public void start(Promise<Void> start) {
      ...
      retriever.getConfig().onComplete(json -> {
        if (json.succeeded()) {
          ...
          ipDomic = (String) ds2.get("url");
          urlDomic = (String) ds2.get("ip");
      ...
      log.info("bcsaUrl : " + bcsaUrl);
      log.info("bcsaIp : " + bcsaIp);
      log.info("bcsaPort : " + bcsaPort);
    ...
    private <T> void domicCharge(Message<T> tMessage) {
      ...
      accountBalance(bodyIn).onComplete(bcsa -> {
          JsonObject bcsac = (JsonObject) bcsa.result();
          if(bcsac.getInteger("code") == 1000) {
              if (bcsac.getFloat("ppalSdoRetTot") > 0.0) {
                client.post(31117, "180.183.170.71", "/api/message2").sendJson(bodyIn, ar -> {
                  if (ar.succeeded()) {
                    HttpResponse<Buffer> response = ar.result();
                    log.info("✅ return pic with HTTP response with status " + response.statusCode());
                    JsonObject body = response.body().toJsonObject();
                ...
                log.info("no se ejecta la operacion de cobranza por falta de saldo");
                JsonObject body = new JsonObject()
                  .put("code", 1020)
            ...
            log.info("no se ejecta la operacion de cobranza por : " + bcsac.getString("message"));
            JsonObject body = new JsonObject()
              .put("code", 1020)
              .put("message", "no se realiza la operacion de cobranza por : " + bcsac.getString("message"));
            tMessage.reply(body);
        ...
        if(bcsa.succeeded()) {
          ...
          if(bcsaR.getInteger("code") == 1000) {
            ...
            if (jsonMap.getFloat("ppalSdoFinal") > 0.0) {
              ...
              bodyInCharge.put("recibos",           "1");
              bodyInCharge.put("importe",           1.0);
